...
bot = telebot.TeleBot('6432703934:AAGm_kQ0tYkoxierJSfP_m6O3NnRx3CeSgk')
HEADERS = {
...
def parse_car_names(link):
    ...
    if html_code.status_code == 200:
        ...
        list_of_names = [i.split('"')[1].split('/')[-1] for i in car_names]
...
class Parser:
    ...
    def save_cars_to_csv(self):
        ...
        data = [{
            'Car_name': i['title'],
            'Price_tg': i['price_tg'].replace('\xa0', '.'),
            'City': i['city'],
            'Description': i['description'],
            'Link': i['link']
        } for i in self.cars]
        df = pd.DataFrame(data)
        df.to_csv(file_name, sep=';', encoding='utf-16', index=False)
